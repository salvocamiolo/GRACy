...
while True:
	...
	if validate == 1:
		if len(fields[3]) >1 or len(fields[4])>1: #an indel
			...
			refAllele = refSeq[int(fields[1])-25:int(fields[1])-2]+fields[3]+refSeq[int(fields[1]):int(fields[1])+26-len(fields[3])]
			altAllele = refSeq[int(fields[1])-25:int(fields[1])-2]+fields[4]+refSeq[int(fields[1]):int(fields[1])+26-len(fields[4])]
